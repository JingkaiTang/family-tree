...
- [ ] 同一根族跨代连续聚合，任一代都没有其他根族家庭插入其列域。
- [ ] 根族之间只有留白和隐式颜色/根家庭强调，没有大背景、标题或根标签。
- [ ] 当前夫妻作为一个单位排版和移动；跨根夫妻卡片按来源根空间方向排序。
- [ ] pair bridge 和 dense island 能覆盖多个子代互婚、三根环和重叠签名。
- [ ] 当前收养跟随收养根；历史/secondary/step/godparent 不改变根或主几何。
- [ ] 卡片和家庭单位没有重叠，主关系线不穿过无关卡片。
- [ ] 不同家庭 route owner 没有正长度共线或假 T；点交叉都有 line bridge。
- [ ] 普通、桥接、根域拖拽分别只写入对应 V4 排序偏好。
- [ ] 恢复默认布局清空三种排序，恢复默认缩放并聚焦 viewpoint 或场景中心。
- [ ] 新增更老祖先后根能向上迁移，同时尽量继承旧颜色、顺序和位置。
- [ ] `rootMemberId` 只影响断开 component 优先级，`viewpointId` 只影响焦点。
- [ ] 500 人性能、全量测试、typecheck 和 build 通过。
- [ ] 旧 cluster/order/compact 生产路径和兼容 action 已删除，无新增依赖。
