...
> **历史设计记录。** 本文中的测试数量、工具选择和“已知 BUG”表仅描述 2026-05-31 当时的状态，不是当前缺陷清单。当前结果以默认分支的测试、CI 和 GitHub Issues 为准。
...
10. SearchBar 组件集成测试 (9 用例)
    - 搜索匹配：姓名 / nickname / occupation
    - 无匹配提示、空输入不显示下拉
    - jump 交互 → ui.setSelected + onJump 回调

11. PhotoCropper 组件基本测试 (3 用例)
    - file=null 不渲染、cancel 事件、挂载/卸载
    - 注：Canvas 裁剪逻辑需浏览器环境，此处仅测基础结构

12. **L3 布局验证场景** (19 用例)
    - 9 种家族结构验证：单人/夫妻/父子/三口/三代/四代/兄弟姐妹/孤立/单亲
    - 验证维度：层级关系、配偶同行、兄弟姐妹同代、无节点重叠、画布尺寸、generation 连续性
